...
- `x11/` holds `.xprofile`, `.xinitrc`, and `.local/bin/dwm-status`;
  `.xinitrc` starts dunst and dwm-status, then execs dwm.
...
- `dwm/` links `config.h` into the dwm source clone at `~/dev/dwm`
  ...
  in `~/dev/dwm` after changing it. The clone also carries a small local
  patch publishing each client's tag as `_NET_WM_DESKTOP`, which
  `claude/.claude/tmux-notify.sh` reads to put the dwm tag in
  notifications.
